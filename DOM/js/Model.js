...
function MazeModel(numW, numH) {   
    ...
    // find path in maze
    this.setPathLabels = function (field, in_out) {
        var d = 0;
        var wave = [in_out[0]];
		var i;
        var k;
        ...
        do {
           ...
           // console.log('================================================');
            d++;
       } while (field[end[1]-1][end[0]-1].label < 0);// if the end is not marked	       
	...
	// finds all paths in maze
	var getPaths = function(x, y, field, path, allPaths) {		
		if (searchSubArray(path, [x, y]) > 0) {
			return;
		}		
		path.push([x, y]);	
		if (field[y-1][x-1].label == 0) {
			allPaths.push(path);
			return;
		}
		...
		if (y > 1) { // top
            if (field[y-2][x-1].hasBottomBorder == false) {
               getPaths(x, y-1, field, path.slice(), allPaths);
            }			
        }	 
		
		if (x < numW) { // right
            if (field[y-1][x-1].hasRightBorder == false) {
                getPaths(x+1, y, field, path.slice(), allPaths);
            }			
        }
	...
	this.findAllPaths = function(field, end) {
		var paths = [];
		getPaths(end[0], end[1], field, [], paths);		
		return paths;		
	}
